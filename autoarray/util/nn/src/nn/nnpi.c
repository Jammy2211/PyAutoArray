...
void nnhpi_interpolate_get_weights(nnhpi* nnhpi, point* p, double* weight_out, int* neighbor_index,
	   	int max_nneighbor, int point_index)
{
    nnpi* nnpi = nnhpi->nnpi;
    //delaunay* d = nnpi->ds->d;
    hashtable* ht_weights = nnhpi->ht_weights;
	...
	count = 0;
    for (i = 0; i < weights->nvertices; ++i) {
		if (count < weights->nvertices){
			if (weights->weights[i] < nnpi->wmin) {
				//p->z = NaN;
				return;
			}
			//p->z += d->points[weights->vertices[i]].z * weights->weights[i];
			weight_out[point_index * max_nneighbor + count] = weights->weights[i];
			neighbor_index[point_index * max_nneighbor + count] = weights->vertices[i];
			//printf("weights vertices: %d", weights->vertices[i]);
			//printf("vertices: %d\n", neighbor_index[point_index * max_nneighbor + count]);
			//printf("values: %f\n", d->points[weights->vertices[i]].z);
			count++;
		} else {
			printf("neighour number exceed maximum!");
			fflush(stdout);
			abort();
			exit(1);
		}
...
int nnhpi_interpolate_output_marker(nnhpi* nnhpi, point* p)
{
    nnpi* nnpi = nnhpi->nnpi;
    delaunay* d = nnpi->ds->d;
    hashtable* ht_weights = nnhpi->ht_weights;
