...
try:
    _file = os.path.realpath(os.path.dirname(__file__)) + '/nn_c_sources/nn/libnnhpi_customized.so'  
    ...
    def natural_interpolation_weights(x_in, y_in, x_target, y_target, max_nneighbours):
        ...
        flag = interpolate_weights_from(
            ...
            ctypes.c_int(nin),
            (x_target.astype(np.double)).ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            (y_target.astype(np.double)).ctypes.data_as(ctypes.POINTER(ctypes.c_double)),
            ctypes.c_int(nout),
        ...
        return weights_out.reshape((nout, max_nneighbours)), neighbour_indexes_out.reshape((nout, max_nneighbours))
    ...
    def natural_interpolation(x_in, y_in, z_in, x_target, y_target):
        ...
        for i in range(nout):
            if z_marker[i] == -1:
                cloest_point_index = np.argmin((x_in - x_target[i])**2.0 + (y_in - y_target[i])**2.0)
