...
def noise_map_from_data_and_exposure_time_map(data, exposure_time_map):
    """ Estimate the noise-map value in every data-point of data, by converting the data to units of counts and 
...
def noise_map_with_offset_values_added(noise_map, scatter=0.001):
    return noise_map + scatter * np.random.uniform(size=noise_map.shape_1d)
...
def background_noise_map_from_edges_of_image(image, no_edges):
    ...
    into a histogram, with a Gaussian profiles to this histogram, such that its standard deviation (sigma) gives an
    estimate of the background noise.

    The background noise-map on the same 2D array as the image is returned.
