...
class AbstractSettingsDataset:
    def __init__(
        ...
        grid_pixelized_class: Optional[Union[Type[Grid1D], Type[Grid2D]]] = None,
        sub_size: int = 2,
        sub_size_pixelized: int = 2,
        ...
        self.grid_pixelized_class = grid_pixelized_class
        self.sub_size = sub_size
        self.sub_size_pixelized = sub_size_pixelized
    ...
    def grid_pixelized_from(self, mask) -> Union[Grid1D, Grid2D]:

        return grid_via_grid_class_from(
            mask=mask,
            grid_class=self.grid_pixelized_class,
            fractional_accuracy=self.fractional_accuracy,
            relative_accuracy=self.relative_accuracy,
            sub_steps=self.sub_steps,
        )
    ...
    def __init__(
        ...
        if conf.instance["general"]["structures"]["use_dataset_grids"]:
            ...
            mask_inversion = mask.mask_new_sub_size_from(
                mask=mask, sub_size=settings.sub_size_pixelized
            )

            self.grid_pixelized = settings.grid_pixelized_from(mask=mask_inversion)
