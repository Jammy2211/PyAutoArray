...
def test__visuals_in_constructor_use_imaging_and_include(imaging_7x7):
    ...
    assert imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).origin == (
        1.0,
        1.0,
    )
    ...
    assert (
        imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).mask
        == imaging_7x7.image.mask
    ).all()

    assert imaging_plotter.visuals_2d.border == None
    assert (
        imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).border
        == imaging_7x7.image.mask.border_grid_sub_1.binned
    ).all()

    assert imaging_plotter.visuals_2d.vector_field == 2
    assert imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).vector_field == 2
    ...
    assert imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).mask == None
    assert imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).border == None
    assert imaging_plotter.get_2d.via_mask_from(mask=imaging_7x7.mask).vector_field == 2
