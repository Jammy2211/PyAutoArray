...
def test__elliptical_properties_and_patches():
    ...
    assert isinstance(vector_field.ellipticities, aa.ValuesIrregularGrouped)
    assert vector_field.ellipticities.in_1d_list == [1.0, 1.0, np.sqrt(2.0)]

    assert isinstance(vector_field.semi_major_axes, aa.ValuesIrregularGrouped)
    assert vector_field.semi_major_axes.in_1d_list == pytest.approx(
        [6.0, 6.0, 7.242640], 1.0e-4
    )

    assert isinstance(vector_field.semi_minor_axes, aa.ValuesIrregularGrouped)
    assert vector_field.semi_minor_axes.in_1d_list == pytest.approx(
        [0.0, 0.0, -1.242640], 1.0e-4
    )

    assert isinstance(vector_field.phis, aa.ValuesIrregularGrouped)
