...
def test__individual_attributes_are_output(interferometer_7, plot_path, plot_patch):
    ...
    interferometer_plotter.visibilities(interferometer=interferometer_7)

    assert path.join(plot_path, "visibilities.png") in plot_patch.paths

    interferometer_plotter.noise_map(interferometer=interferometer_7)

    assert path.join(plot_path, "noise_map.png") in plot_patch.paths

    interferometer_plotter.u_wavelengths(interferometer=interferometer_7)

    assert path.join(plot_path, "u_wavelengths.png") in plot_patch.paths

    interferometer_plotter.v_wavelengths(interferometer=interferometer_7)

    assert path.join(plot_path, "v_wavelengths.png") in plot_patch.paths

    interferometer_plotter.uv_wavelengths(interferometer=interferometer_7)

    assert path.join(plot_path, "uv_wavelengths.png") in plot_patch.paths

    interferometer_plotter.amplitudes_vs_uv_distances(interferometer=interferometer_7)

    assert path.join(plot_path, "amplitudes_vs_uv_distances.png") in plot_patch.paths

    interferometer_plotter.phases_vs_uv_distances(interferometer=interferometer_7)
...
def test__subplot_is_output(interferometer_7, plot_path, plot_patch):
    ...
    interferometer_plotter.subplot_interferometer(interferometer=interferometer_7)
