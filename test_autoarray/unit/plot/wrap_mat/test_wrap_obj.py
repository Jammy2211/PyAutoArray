...
def test__all_load_correctly():
    ...
    positions_scatter.scatter_grid(grid=aa.Grid.uniform(shape_2d=(3, 3), pixel_scales=1.0))
    ...
    pixelization_grid_scatter.scatter_grid(grid=aa.Grid.uniform(shape_2d=(3, 3), pixel_scales=1.0))
    
    parallel_overscan_line = aplt.ParallelOverscanLine()
    parallel_overscan_line.draw_y_vs_x(y=[1.0, 2.0, 3.0], x=[1.0, 2.0, 3.0], plot_axis_type="linear")
    
    serial_overscan_line = aplt.SerialOverscanLine()
    serial_overscan_line.draw_y_vs_x(y=[1.0, 2.0, 3.0], x=[1.0, 2.0, 3.0], plot_axis_type="semilogy")
    
    serial_prescan_line = aplt.SerialPrescanLine()
    serial_prescan_line.draw_y_vs_x(y=[1.0, 2.0, 3.0], x=[1.0, 2.0, 3.0], plot_axis_type="loglog")