...
def test__visuals_in_constructor_use_imaging_and_include(fit_imaging_7x7):
    ...
    assert fit_imaging_plotter.extractor_2d.via_fit_from(
        fit=fit_imaging_7x7
    ).origin == (1.0, 1.0)

    assert (
        fit_imaging_plotter.extractor_2d.via_fit_from(fit=fit_imaging_7x7).mask
        == fit_imaging_7x7.image.mask
    ).all()
    assert (
        fit_imaging_plotter.extractor_2d.via_fit_from(fit=fit_imaging_7x7).border
        == fit_imaging_7x7.image.mask.border_grid_sub_1.binned
    ).all()
    assert (
        fit_imaging_plotter.extractor_2d.via_fit_from(fit=fit_imaging_7x7).vector_field
        == 2
    )
    ...
    assert (
        fit_imaging_plotter.extractor_2d.via_fit_from(fit=fit_imaging_7x7).mask == None
    )
    assert (
        fit_imaging_plotter.extractor_2d.via_fit_from(fit=fit_imaging_7x7).border
        == None
